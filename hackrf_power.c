...
void usage(void)
{
	fprintf(stderr,
		...
		"\t[-g vga tuner_gain (default: 30)]\n"
		"\t[-G lna_tuner_gain (default: 16)]\n"
...
/* flesh out the tunes[] for scanning */
{
	...
	for (i=0; i < c.hops; i++) {
		...
		/* copy common values */
		ts->rate = c.bw_needed;
		ts->lnaGain = ms->lnaGain;
		ts->vgaGain = ms->vgaGain;
		ts->bin_e = c.bin_e;
		ts->samples = 0;
...
void retune(hackrf_device *d, int freq)
{
	hackrf_set_freq(d, (uint64_t)freq);
	/* wait for settling and flush buffer */
	usleep(5000);
	hackrf_buffer_reset();
}

void rerate(hackrf_device *d, int rate)
{
  static int last_rate = -1;
  if ( rate != last_rate ) {
    /* no way to get current sample rate, so just set it */
    fprintf(stderr,"rerate %d\n", tunes[0].rate);
    hackrf_set_sample_rate(dev, (double) rate);
    last_rate = rate;
  }
}

 void regain(hackrf_device *d, int lnaGain, int vgaGain)
{
  static int last_lnaGain = -1;
  static int last_vgaGain = -1;
  if ( last_lnaGain != lnaGain ) {
    hackrf_set_lna_gain(d, lnaGain);
    last_lnaGain = lnaGain;
  }
  if ( last_vgaGain != vgaGain ) {
    hackrf_set_vga_gain(d, vgaGain);
    last_vgaGain = vgaGain;
  }
}
...
void scanner(void)
{
	...
	for (i=0; i<tune_count; i++) {
		...
		ts = &tunes[i];
		fft_buf = ts->fft_buf;
		regain(dev, ts->lnaGain, ts->vgaGain);
		rerate(dev, ts->rate);
		retune(dev, ts->freq);
...
void init_misc(struct misc_settings *ms)
{
	...
	ms->comp_fir_size = 0;
	ms->crop = 0.0;
	ms->lnaGain = GAIN_LNA_DEFAULT;
	ms->vgaGain = GAIN_VGA_DEFAULT;
	ms->gpsWait = 5;
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "f:i:s:r:t:d:g:p:e:w:T:c:F:1EPLD:Oh")) != -1) {
		...
		case 'd':
		  ...
		  fprintf(stderr,"To be implemented..\n"); exit(1);
			...
			break;
		case 'g':
		  ms.vgaGain = (int)(atof(optarg));
			break;
		case 'G':	
		  ms.lnaGain = (int)(atof(optarg));
			break;
		case 'c':
	...
	fprintf(stderr,"Start GPS..\n");
      ...
      regain(dev, ms.lnaGain, ms.vgaGain);
	...
	/* actually do stuff */
	rerate(dev, tunes[0].rate);

	/*
	 * Enable hackrf and transfers..
	 */
	hackrf_start_rx(dev, hackrf_rx_callback, NULL);
	//	if ( ! hackrf_start_rx(dev, hackrf_rx_callback, NULL) ) {
	//	  fprintf(stderr,"Can't start callback\n");
	//	  exit(1);
	//	}
