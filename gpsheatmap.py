...
for line in raw_data():
    ...
    low  = int(line[COL_FLO]) + args.offset_freq
    high = int(line[COL_FHI]) + args.offset_freq
    step = float(line[COL_FSTEP])
    t = line[0] + ' ' + line[1]
    if '-' not in line[0]:
        t = line[0]
    ...
    zs = line[COL_DATA+start_col:COL_DATA+stop_col+1]
    if not zs:
        continue
...
def collate_row(x_size):
    ...
    for line in raw_data():
        ...
        zs = floatify(line[COL_DATA+start_col:COL_DATA+stop_col+1])
        if t != old_t:
            yield old_t, row
            row = [0.0] * x_size
        old_t = t
